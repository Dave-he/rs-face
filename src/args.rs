...
pub struct DetectOpts {
    ...
    /// 视频概览模式: 整个视频只输出 1 张代表图 + 1 行 manifest。用于快速浏览。
    pub video_summary: bool,
}
...
pub fn parse() -> Result<Command, BoxError> {
    ...
    match args[0].as_str() {
        ...
        s => Err(format!("未知子命令: {} (可用: detect, train, recognize, benchmark, name, collect, info)", s).into()),
    }
}
...
fn parse_collect(args: &[String]) -> Result<Command, BoxError> {
    let mut tracks = PathBuf::new();
    let mut frames_dir = PathBuf::new();
    let mut out = PathBuf::from("./dataset");
    let mut samples_per_track = 5;
    let mut i = 0;
    while i < args.len() {
        match args[i].as_str() {
            "--tracks" => { tracks = take_path(args, &mut i)?; }
            "--frames" => { frames_dir = take_path(args, &mut i)?; }
            "--out" | "-o" => { out = take_path(args, &mut i)?; }
            "--samples" => { samples_per_track = take_str(args, &mut i)?.parse()?; }
            "--help" | "-h" => { print_help(); return Ok(Command::Help); }
            other => return Err(format!("未知 collect 参数: {}", other).into()),
        }
        i += 1;
    }
    if tracks.as_os_str().is_empty() || frames_dir.as_os_str().is_empty() {
        return Err("collect 必须指定 --tracks 和 --frames".into());
    }
    Ok(Command::Collect(CollectOpts { tracks, frames_dir, out, samples_per_track }))
}
